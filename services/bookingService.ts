...
export const saveBookingToSupabase = async (booking: BookingData): Promise<{ success: boolean; error?: string; id?: string }> => {
    try {
        ...
        if (booking.phone) insertData.phone = booking.phone;
        ...
        if (booking.language) insertData.language = booking.language;
        if (booking.amount_paid !== undefined) insertData.amount_paid = booking.amount_paid;
        if (booking.stripe_payment_id) insertData.stripe_payment_id = booking.stripe_payment_id;
